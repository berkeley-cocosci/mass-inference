"""Circular statistics module, written by Peter Battaglia (pbatt@mit.edu)
and Jessica Hamrick (jhamrick@berkeley.edu).
...
def _corrcc(alpha1, alpha2, nanrobust, axis=None):
    # FISHER, N. & LEE, A. (1983). A correlation coefficient for
    # circular data. Biometrika. 70 (2), 327-332.
...
def vmpar(alpha, axis=None, w=None, d=0, nancheck=True):
    ...
    if nancheck and np.isnan(alpha).any():
        raise(ValueError,
              ...
              "the mean robust to NaNs, please use the nancorrcc "
              "function instead.")
...
def kappa(alpha, axis=None, w=None, d=0, nancheck=True):
    ...
    if nancheck and np.isnan(alpha).any():
        raise(ValueError,
              ...
              "mean robust to NaNs, please use the nancorrcc "
              "function instead.")
