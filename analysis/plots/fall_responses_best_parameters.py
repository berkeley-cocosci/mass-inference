...
def plot(results_path, fig_path):
    ...
    fig, ax = plt.subplots()
    cax = ax.imshow(
        corrs, cmap='gray', interpolation='nearest', vmin=0.6, vmax=0.8,
        origin='lower')
    ...
    fig.colorbar(cax, ticks=[0.6, 0.65, 0.7, 0.75, 0.8])
    ...
    pths = [fig_path.joinpath("fall_responses_best_parameters.%s" % ext)
            for ext in ('png', 'pdf')]
    for pth in pths:
