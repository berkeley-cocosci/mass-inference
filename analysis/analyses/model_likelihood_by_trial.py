...
def make_model_df(arr, trials, hyps, kappa0, pid):
    ...
    new_df['pid'] = pid
    new_df = new_df.set_index(['pid', 'trial', 'hypothesis'])
    return new_df
...
def run(results_path, seed):
    ...
    old_store = pd.HDFStore('results/model_likelihood.h5')
    ...
    for key in old_store.keys():
        ...
        for (version, kappa0), pids in participants.groupby(level=['version', 'kappa0']):
            ...
            df = np.asarray(llh.ix[kappa0])

            for pid in pids:
                order, i = orders[pid]
                dfi = df[i]

                model = make_model_df(dfi, order, hyps, kappa0, pid)
                store.append('{}/{}'.format(key, version), model)
