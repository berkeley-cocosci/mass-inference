...
def mv_gaussian_kde(vals, h=0.5, pu=0.05, n=48, u=0.1):
    k = rvs.MVGaussian(np.zeros(vals.shape[-1]), np.eye(vals.shape[-1]))
    n = np.sum(np.any(~np.isnan(vals), axis=-1), axis=-1)[..., None]
    def f(x):
        diff = x[..., None, :] - vals[..., None, :, :]
