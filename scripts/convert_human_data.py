...
OLDPATH = path("../../mass-inference-data/human-raw/")
NEWPATH = path("../../mass-inference-data/human/")
...
for dataset in OLDPATH.listdir():
    ...
    data_path = os.path.join(NEWPATH, dataset_name)
    ...
    json_path = os.path.join(data_path, "datapackage.json")
    metadata = {
        # required
        'name': dataset_name,
    ...
    if dataset_type == "mass_learning":
        metadata['contributors'].insert(1, {
        ...
        batch.to_csv(batch_path)

        with open(batch_path, 'r') as fh:
            batch_file = fh.read()
        batch_hash = hashlib.md5(batch_file).hexdigest()
        batch_size = batch_path.getsize()

        metadata['resources'].insert(0, {
            'name': "turk",
            'format': 'csv',
            'path': "./%s" % batch_name,
            'mediaformat': 'text/csv',
